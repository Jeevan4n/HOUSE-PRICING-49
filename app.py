...
regmodel = pickle.load(open(r"C:\Users\JEEVAN\Downloads\regmodel (2).pkl", 'rb'))
# Load the dataset to extract feature names
df = pd.read_csv(r"C:\Users\JEEVAN\OneDrive\Desktop\Housepricing\HOUSE-PRICING-49\manu.csv")  # Replace with your actual dataset file
...
def predict_api():
    ...
    output = regmodel.predict(data)  # Make prediction
...
def predict():
    ...
    final_input = np.array(data).reshape(1, -1)  # Scale the input
...
if __name__ == "__main__":
    app.run(debug=False)  # Run the Flask app with debugging enabled
